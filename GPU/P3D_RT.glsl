...
bool hit_world(Ray r, float tmin, float tmax, out HitRecord rec)
{
    ...
    {
        hit = true;
        rec.material = createMetalMaterial(vec3(0.7, 0.6, 0.5), 0.7);
    }
...
vec3 directlighting(pointLight pl, Ray r, HitRecord rec){
   ...
   float diffuse, specular;
    ...
    vec3 L = normalize((pl.pos - rec.pos)); //Light Direction
    if(dot(L, rec.normal) > 0.0) {

        vec3 hit_point = rec.pos + epsilon * rec.normal;
        Ray rShadow = createRay(hit_point, L);
        float len = length(pl.pos - rec.pos); //distance between the hit point and the light source

        // check if it's in shadow
        if(hit_world(rShadow, 0.0, len, dummy))
        {
            return colorOut;
        }
...
vec3 rayColor(Ray r)
{
    ...
    for(int i = 0; i < MAX_BOUNCES; ++i)
    {
        if(hit_world(r, 0.001, 10000.0, rec))
        {
            ...
            {      
                col *= atten; // Apply the attenuation of the scattered ray to the color

                r = scatterRay; // Update the primary ray to be the scattered ray direction
                throughput *= atten; // Update the throughput with the attenuation factor
